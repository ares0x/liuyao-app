...
//     palace: string;
//     shi: number;
//     nature: string; // 卦象特质 (如：上上卦、刚健中正)
//     guaci: string; // 卦辞
//     xiangci: string; // 象辞
//     explanation: string; // 核心详解 (包含白话、运势、事业婚恋等)
// }
...
export const HEXAGRAM_DATA: Record<string, HexagramData> = {
    ...
    "111111": {
        ...
        palace: "乾",
        shi: 5,
        nature: "上上卦 · 刚健中正", // 提取顶部的简述
        guaci: "元亨利贞。",
        xiangci: "天行健，君子以自强不息。",
        // 使用模板字符串 `` 支持换行，这里使用了简单的 Markdown 标记
        explanation: `**【白话解释】**
乾卦：大吉大利，吉利的贞卜。
...
**【卦意详解】**
此卦为同卦（下乾上乾）相叠。象征天，喻龙（德才的君子），又象征纯粹的阳和健，表明兴盛强健。乾卦是根据万物变通的道理，以“元、亨、利、贞”为卦辞，示吉祥如意，教导人遵守天道的德行。
...
* **寻人**：已远走高飞，不易找到，在西北方向。`,
    ...
    **【卦意详解】**
    此卦为异卦（下巽上乾）相叠。乾为天，巽为风。天下有风，吹遍大地，阴阳交合，万物茂盛。姤（gǒu）卦与夬卦相反，互为“综卦”。姤即遘，阴阳相遇。但五阳一阴，不能长久相处，阴长阳消，诸事阻滞。
    ...
    **【大师解卦】**
    * **解释**：二女争一男，代表邂逅、不期而遇，非预想中的事情发生。
    * **特性**：杞人忧天，虎头蛇尾，易感伤，有艺术美学之天分，较理想化。
    * **运势**：阴长阳衰，鸿运中衰。男人谨防因色受祸，诸事不如意，不能上进，沉于女色之凶象也。凡事进取宜慎重。
    ...
    **【卦意详解】**
    此卦为同卦（下泽上泽）相叠。泽为水。两泽相连，两水交流，上下相和，团结一致，朋友相助，欢欣喜悦。兑为悦也。同秉刚健之德，外抱柔和之姿，坚行正道，导民向上。
    ...
    * **失物**：遗失物似为金属或金钱，有望失而复得，但是迟一点。且多数已损毁或损失。`,
    ...
    "110111": {
      ...
      **【卦意详解】**
      此卦为异卦（下乾上巽）相叠，乾为天；巽为风。喻风调雨顺，谷物滋长，故卦名小畜（蓄）。力量有限，须待发展到一定程度，才可大有作为。
      密云不雨之象，蓄养待进之意。
    ...
    **【卦意详解】**
    此卦为异卦（下乾上坤）相叠，乾为天，为阳；坤为地，为阴。阴阳交感，上下互通，天地相交，万物纷纭。反之则凶。万事万物，皆对立，转化，盛极必衰，衰而转盛，故应时而变者泰（通）。
    否极泰来，鸿运当头，但须防乐极生悲。
    ...
    * **开业**：开业者吉利之象。`,
    ...
    "000100": {
        ...
        explanation: `**【白话解释】**
    谦卦：通泰。筮遇此卦，君子将有所成就。
    《象辞》说：本卦外卦为坤为地，内卦为艮为山，地中有山，内高外卑，居高不傲，这是谦卦的卦象。君子观此卦象，以谦让为怀，裁取多余，增益缺乏的，衡量财物的多寡而公平施予。
    ...
    **【卦意详解】**
    此卦为异卦（下艮上坤）相叠，艮为山，坤为地。地面有山，地卑（低）而山高，是为内高外低，比喻功高不自居，名高不自誉，位高不自傲。这就是谦。
    谦虚忍让者前途大好，骄横者必招败。谦受益，满招损。
    ...
    **【卦意详解】**
    此卦为异卦（下坎上坤）相叠。“师”指军队。坎为水、为险；坤为地、为顺，喻寓兵于农。兵凶战危，用兵乃圣人不得已而为之，但它可以顺利无阻碍地解决矛盾，因为顺乎形势，师出有名，故能化凶为吉。
    行险而顺，包容别人，修行待时。
    ...
    **【卦意详解】**
    此卦为异卦（下兑上坤）相叠。坤为地；兑为泽，地高于泽，泽容于地。喻君主亲临天下，治国安邦，上下融洽。
    大驾光临，贵人来到之意。主吉象。临又为「君王临幸」之意，是上者对下者之象。
    ...
    **【卦意详解】**
    此卦为异卦（下离上坤）相叠。离为明，坤为顺；离为日；坤为地。日没入地，光明受损，前途不明，环境困难，宜遵时养晦，坚守正道，外愚内慧，韬光养晦。
    明夷代表火入坑中，阳气被阴气所伤害，诸事不宜，运背之时。
    ...
    **【卦意详解】**
    此卦为异卦（下震上坤）相叠。震为雷、为动；坤为地、为顺，动则顺，顺其自然。动在顺中，内阳外阴，循序运动，进退自如，利于前进。
    一阳来复，万事苏伸，寓动于顺，循环往复。
    ...
    * **决策**：性格活泼，精力旺盛，但因急躁情绪会造成事业损失。一定要冷静思考，勇于改正错误，必能重振事业。此时应抓住时机，明确方向，灵活机敏，脚踏实地改变处境。一旦出现意外，宜当机立断，暂行退让。
    ...
    **【卦意详解】**
    此卦是异卦相叠（下巽上坤）。坤为地、为顺；巽为木、为逊。大地生长树木，逐步成长，日渐高大成材，喻事业步步高升，前程远大，故名“升”。
    升腾上进，畅行其志；出暗向明，遂渐升进。
    ...
    **【卦意详解】**
    此卦为同卦（下坎上坎）相叠。坎为水、为险，两坎相重，险上加险，险阻重重。一阳陷二阴。所幸阴虚阳实，诚信可豁然贯通。虽险难重重，却方能显人性光彩。
    艰难危险，重险重陷；事多困阻，谨慎行事。
    ...
    **【卦意详解】**
    这个卦是同卦（下坤上坤）相叠，阴性。象征地（与乾卦相反），顺从天。承载万物，伸展无穷无尽。坤卦以雌马为象征，表明地道生育抚养万物，而又依天顺时，性情温顺。
    ...
    **【卦意详解】**
    此卦是异卦（下坤上乾）相叠，其结构同泰卦相反，系阳气上升，阴气下降，天地不交，万物不通。它们彼此为“综卦”，表明泰极而否，否极泰来，互为因果。
    万物闭塞之象，上下不合，诸事不顺，凡事宜忍，须待时运好转而有为。
    ...
    **【卦意详解】**
    此卦是异卦（下坎上乾）相叠。同需卦相反，互为“综卦”。乾为刚健，坎为险陷。刚与险，健与险，彼此反对，定生争讼。争讼非善事，务必慎重戒惧。
    代表诉讼、争端之象。主大凶象。事情冲突到必须见官上法院，可能必须以法律途径来解决了！
    ...
    * **决策**：争强好胜，不安于现状，为改变命运和超越他人而奋斗。头脑聪颖，反应敏捷，有贵人相助。但缺乏持之以恒的毅力，容易露出锋芒，得罪他人，带来诉讼之灾。宜承认现实，顺其自然，知足，适可而止。
    ...
    **【卦意详解】**
    这个卦是异卦（下兑上乾）相叠，乾为天；兑为泽，以天喻君，以泽喻民。君上民下，各得其位。兑柔遇乾刚，所履危。
    履意为实践，卦义是脚踏实地的向前进取的意思。履虎尾，不咥（咬）人，结果吉利。
    ...
    **【大师解卦】**
    * **解释**：只要履行契约承诺则吉。
    * **特性**：战战兢兢、如履薄冰，竞争，积极，冒险，热枕，独立，自立自强，有骨气。
    ...
    **【卦意详解】**
    此卦是异卦（下离上乾）相叠，乾为天，为君；离为火，为臣民百姓，上天下火，火性上升，同于天，上下和同，同舟共济，人际关系和谐，天下大同。
    代表有志一同，相和而成。最适合找人合伙事业或寻求认同自己理念的吉卦。
    ...
    * **决策**：精力充沛，富有上进心，可有作为。但务必勤勉努力，诚心诚恳，奋发向上，刻苦钻研。不可因小有成就而得意忘形。不必追求所得，而应追求实干。遵循规律办事，等待时机再行动。尤其应清除非份的念头。
    ...
    **【卦意详解】**
    此卦是异卦（下坎上艮）相叠，艮是山的形象，喻止；坎是水的形象，喻险。卦形为山下有险，仍不停止前进，是为蒙昧，故称蒙卦。但因把握时机，行动切合时宜，因此，具有启蒙和通达的卦象。
    智慧未开，蒙昧闭塞；犹豫不决，缺乏果断。若能顺贤师良友之教，启其聪明则亨通。
    ...
    **【卦意详解】**
    此卦是异卦（下乾上艮）相叠。乾为天，刚健；艮为山，笃实。畜者积聚，大畜意为大积蓄。为此不畏严重的艰难险阻，努力修身养性以丰富德业。
    以阳畜阴，制止欲进；坚守正道，先凶后吉。
    ...
    * **决策**：喜欢注重外表，缺乏追求内在气质的恒心。生性较为浮躁，好务虚名，往往因此而碰壁，或半途而废。在事实的教育下，改变不切实际的做法，明察社会和人生，会有所收敛，变得自持，注重自身修养，成全事业。
    ...
    * **开业**：开业者不利，须再待时。`,
    ...
    《象辞》说：本卦上卦为艮为山，下卦为巽为风，贤人如山居于上，宣布德教施于下，所谓山下有风，这是巽卦的卦象。君子观此卦象，取法于吹拂万物的风，从而振救万民，施行德教。
    ...
    * **决策**：头脑聪明、灵活，性格开朗，以谦虚的态度处世，会受到各个方面的欢迎，也会得到上级的重视。谦虚不可过度，应以刚健中正为前提，决不可表现为懦弱和虚伪。
    ...
    **【卦意详解】**
    此卦是异卦（下坤上坎）相叠，坤为地；坎为水。水附大地，地纳河海，相互依赖，亲密无间。此卦与师卦完全相反，互为综卦。它阐述的是相亲相辅，宽宏无私，精诚团结的道理。
    水行地上，亲比欢乐；人情亲顺，百事无忧。
    ...
    * **决策**：前途光明，有雄心大志，且可实现。为此需要积蓄实力，等待时机，大器晚成。本人具有坚强的意志，冷静静的头脑。前进途中会遇到困难和险阻，必须十分谨慎，坦然对待小人的中伤，在灾祸在面前能镇静自若。
    ...
    * **决策**：初始困难，但若具有坚忍不拔的毅力和锲而不舍的奋斗精神，前途不可估量，但往往不为他人理解而陷于孤独苦闷，事业会因此处于困难状态，需要得到贤德之人的帮助才能摆脱。
    ...
    **【大师解卦】**
    * **解释**：卦相奇特，角龙升天行大运。代表决定或缺失，所迟疑的事情，一定要有个决定。
    ...
    * **决策**：吉祥如意。人际关系好，善于交际。以真诚的态度，无私的心去处世，不会有不利的地方。与他人合作应坚持原则。办事不可优柔寡断。不顺利时，应停止行动，总结经验，以图发展。
    ...
    * **决策**：十分吉利、顺利。以自己的聪明才智，顺应形势，根据时代特点，依理而动，变革渐合时宜的事物，壮大自己的事业。但应谨慎，时机不成熟不可妄动，尤忌急功近利。
    ...
    * **决策**：性格坚强，勇于战胜困难，奋斗向上，前进途中困难和厄运很多，唯有认真对待才能不断取得成功，如放松警惕，骄傲自满，将带来惨重失败。
    ...
    * **决策**：处于矛盾、对应状态。必须注意以宽宏大度、平和、柔顺的态度对待人生。这样才能心平气和，柔而上进。认清合必有离，离必有合；同中有异，异中有同的道理。
    ...
    **【卦意详解】**
    这个卦是异卦（下震上离）相叠。离为阴卦；震为阳卦。阴阳相交，咬碎硬物，喻恩威并施，宽严结合，刚柔相济。噬嗑（shì hé）为上下颚咬合，咀嚼。
    咬碎硬骨，强硬态度；事多困阻，积极谋求。
    ...
    * **开业**：吉利。`,
    ...
    "001100": {
        ...
        xiangci: "山上有雷，小过。君子以行过乎恭，丧过乎衰，用过乎俭。",
        explanation: `**【白话解释】**
    ...
    **【卦意详解】**
    这个卦是异卦（下兑上震）相叠。震为动、为长男；兑为悦、为少女。以少女从长男，产生爱慕之情，有婚姻之动，有嫁女之象，故称归妹。男婚女嫁，天地大义，人的开始和终结。
    违反常理，其道将穷；明察事理，止绝妄念。
    ...
    * **决策**：具有组织能力，适宜担任负责工作，但人生旅途多曲折，必须注意培养自己的预见性和观察力，办事务必公正，实事求是，注意人心动向，关心他人。尤其不可以陷入小人之道和近视行为之中。
    ...
    * **外出**：有利可获。大胆外出行动。
    * **婚恋**：心胸开阔，乐于助人。人际关系好，得到众人的支持。自己贵在树立恒心，克服三心二意。尤其要向比自己优秀的人学习，借助对方的力量，充实和发展自己。
    * **决策**：树立必要的冒险精神。平日要施善于人。因此，即使遇到危险也会得到他人救助。
